...
/**
 * Sends a transactional email via Brevo's HTTPS API (not SMTP).
 * Render's free tier blocks outbound traffic to SMTP ports (25/465/587),
 * so nodemailer-over-SMTP hangs indefinitely there. The REST API runs
 * over HTTPS (443), which is not blocked.
 */
const sendViaBrevoApi = async ({ senderName, toEmail, subject, html }) => {
  const response = await fetch(BREVO_API_URL, {
    ...
    body: JSON.stringify({
      sender: {
        name: senderName,
        email: process.env.EMAIL_FROM || 'ndakiet1001@gmail.com'
      },
      to: [{ email: toEmail }],
      subject,
      htmlContent: html
  ...
  if (!response.ok) {
    ...
    throw new Error(`Brevo API trả lỗi ${response.status}: ${errorBody}`);
  }
